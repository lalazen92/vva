package vav.cyberspace.viettel.vva.wav;
...
import vav.cyberspace.viettel.vva.log.AppLog;

public class ExtAudioRecorderModified {
	public /*final*/ static int CAPTURE_SAMPLE_RATE_HZ = 16000;
	...
	boolean bCheckSilentVoice = false;
	/**
	 * 
	...
	private AudioRecord.OnRecordPositionUpdateListener updateListener = new AudioRecord.OnRecordPositionUpdateListener() {
		public void onPeriodicNotification(AudioRecord recorder) {
			...
			bufferReadResult = audioRecorder.read(buffer, 0, buffer.length); // Fill buffer
			...
			try {
				...
				sendMessage(2);
				if(getMaxAmplitude() < threshold){
					if(beginTime == -1)
						beginTime = System.currentTimeMillis();
					endTime = System.currentTimeMillis();
					if(beginTime != -1){
						float elapsedTime = (float) (endTime - beginTime)/1000;

						if(elapsedTime >= 1.4 && bCheckSilentVoice == true && bStart == false){
							bWrite = false;
							sendMessage(1);
						}
			...
			if (state == State.INITIALIZING) {
				if ((audioRecorder.getState() == AudioRecord.STATE_INITIALIZED)
					...
					buffer = new byte[framePeriod * bSamples / 8
							* nChannels];
					if (bSamples == 16) {
						wavData = new short[buffer.length / 2];

					} else { // 8bit sample size
						wavData = new short[buffer.length];
					}
					state = State.READY;
				} else {
					Log.e(ExtAudioRecorderModified.class.getName(),
	...
	public void start() {
		...
		if (state == State.READY) {
			...
			bufferReadResult = audioRecorder.read(buffer, 0, buffer.length);
	...
	protected int getStatus(int numOfBytes, int len) {
		AppLog.logString("Read bytes: request/actual: " + len + "/" + numOfBytes);
		if (numOfBytes == AudioRecord.ERROR_INVALID_OPERATION) {
			AppLog.logString("The SpeechRecord object was not properly initialized");
			return -1;
		} else if (numOfBytes == AudioRecord.ERROR_BAD_VALUE) {
			AppLog.logString("The parameters do not resolve to valid data and indexes.");
			return -2;
		} else if (numOfBytes > len) {
			AppLog.logString("Read more bytes than is buffer length:" + numOfBytes + ": " + len);
			return -3;
		} else if (numOfBytes == 0) {
			AppLog.logString("Read zero bytes");
			return -4;
		}
		return 0;
	}
